...
if os.path.exists("ipstools") and os.path.isdir("ipstools"):
    cwd = os.getcwd()
    os.chdir("ipstools")
    execute("git pull origin vcs-pulpino", silent=True)
    os.chdir(cwd)
    import ipstools
else:
    # try to find the ipstools repository
    if "http" in remote:
        if execute("git clone %s/IPApproX.git ipstools -b vcs-pulpino" % (remote)) != 0:
            execute("git clone %s/pulp-tools/IPApproX.git ipstools -b vcs-pulpino" % (server))
    else:
        if execute("git clone %s/IPApproX.git ipstools -b vcs-pulpino" % (remote)) != 0:
            execute("git clone %s:pulp-tools/IPApproX.git ipstools -b vcs-pulpino" % (server))
